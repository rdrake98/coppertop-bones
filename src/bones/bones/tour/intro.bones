...
// we can also tell the kernel to include other libraries - this doens't make names available just add functionality
// to names we have said we want to use
from dm.stats include core
...
(1,2,3) each both (1,2,3) check equals (2,4,6)

// expressions can span multiple lines if subsequent lines after the phrases start are indented
(1,2,3)
    each both (1,2,3)
    check equals (2,4,6)
